function [A, c_old,B_old,fun] = MTL_MMTFL_logistic(X_cell,Y_cell,hyp, opts)
%% Initialization 
% hyp =[gamma1, gamma2)
...
%                 [B_new(:,t),b_fun] = L1_FISTA(Z,Y_cell{t}, hyp(1), opts);                
                [B_new(:,t),b_fun] = L1_FISTA_logistic(Z,Y_cell{t},hyp(1),opts);
            case 2
    ...
    function val = Obj_multiplicative(c,B)
        ...
        for t2=1:T
            val =  val + Loss_logistic(X_cell{t2}, Y_cell{t2}, diag(c) *B(:,t2))/(length(Y_cell{t2}));
